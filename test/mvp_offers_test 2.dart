...
Product _product() {
  return const Product(
    id: 'mvp-offer-product',
    name: 'MVP offer test item',
    description: 'A test product for offer visibility.',
    quality: 'Good',
...
void main() {
  testWidgets('DonationOptions hides open-to-offers toggle for the MVP', (
    tester,
  ) async {
    ...
    expect(find.text(AppStrings.openToOffersText), findsNothing);
  });

  testWidgets('ProductPage hides make-offer button for the MVP', (tester) async {
  ...
  test('FAQ data hides offers category for the MVP', () {
    expect(
      dummyFaqData.where((category) => category.title == AppStrings.offers),
      isEmpty,
    );
  });
}
